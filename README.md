...
- start command: `node --env-file=~/.openship/env/sas.env server.cjs`
...
OpenShip should install dependencies, run `npm run build`, and run `server.cjs` with `PORT=20135`. The launcher sets `NODE_ENV=production` and loads `build/boot.js`. Caddy terminates TLS and proxies `sas.hhdevs.space` to `127.0.0.1:20135`. MongoDB is external; no database files belong in an OpenShip release.
